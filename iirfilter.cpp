...
namespace fir_iirfilter {
    ...
    Mat FIR_IIRFilter::McClellanTransform(Mat fir1d, std::vector<double> abcde,int nbPts)
    {
        if (fir1d.rows % 2 == 0)
        {
            Exception e;
            e.code = -2;
            e.msg = "McClellan transform is only for odd size filter";
            throw e;
        }
        if (abcde.size() != 5)
        {
            Exception e;
            e.code = -2;
            e.msg = "McClellan transform need A, B, C, D and E parameters";
            throw e;
        }
        nbPts = getOptimalDFTSize(nbPts);
        Mat H(nbPts,nbPts,CV_64FC2);
        int halfFilterSize = (fir1d.rows-1)/2;
        double energy=0;
        for (int i = 0; i < nbPts; i++)
        {
            double w1=i*TWOPI/nbPts;
            for (int j = 0; j < nbPts; j++)
            {
                double w2=j*TWOPI/nbPts;
                double F0=1;
                double F = abcde[0] + abcde[1] * cos(w1) + abcde[2] * cos(w2) + abcde[3] * cos(w1+w2) + abcde[4] * cos(w1-w2);
                double F1 = F;
                double t=fir1d.at<double>(halfFilterSize,0);
                t = t+F1*2*fir1d.at<double>(halfFilterSize+1,0);



                for (int k = 2; k <= halfFilterSize; k++)
                {
                    double F2=2*F*F1-F0;
                    F0=F1;
                    F1=F2;
                    t = t+F1*2*fir1d.at<double>(halfFilterSize+k,0);

                }
                H.at<complex<double>>(i,j)=complex<double>(t,0);
                energy+=t*t;
            }

        }
        Mat h;
        idft(H,h,DFT_REAL_OUTPUT|DFT_SCALE);
        Mat fir2d = Mat::zeros(fir1d.rows,fir1d.rows,CV_64FC1);
        h(Rect(0,0,halfFilterSize+1,halfFilterSize+1)).copyTo(fir2d(Rect(halfFilterSize,halfFilterSize,halfFilterSize+1,halfFilterSize+1)));
        h(Rect(h.rows-halfFilterSize,h.rows-halfFilterSize,halfFilterSize,halfFilterSize)).copyTo(fir2d(Rect(0,0,halfFilterSize,halfFilterSize)));
        h(Rect(0,h.rows-halfFilterSize,halfFilterSize,halfFilterSize)).copyTo(fir2d(Rect(halfFilterSize,0,halfFilterSize,halfFilterSize)));
        h(Rect(h.rows-halfFilterSize,0,halfFilterSize,halfFilterSize)).copyTo(fir2d(Rect(0,halfFilterSize,halfFilterSize,halfFilterSize)));
        return fir2d;
    }

    Mat FIR_IIRFilter::UnitaryFilter(Mat l)
    {
        Mat h11d(l.size(),CV_64FC1);
        Mat x;
        if (l.cols==1)
            x = l.t();
        else
        ...
        for (int i=0;i<=nbTapL1/2;i++)
            h11d.at<double>(i, 0) = x.at<double>(0,nbTapL1/2+i);
        for (int i=0;i<nbTapL1/2;i++)
            h11d.at<double>(i+nbTapL1/2+1, 0) = x.at<double>(0,i);
        Mat H11d;
...
/*            if (i>=N/2)
    ...
    FIR_IIRFilter::FIR_IIRFilter(string ftype, int o, double fs, vector<double> frequency)
    {
    ...
    complex<double>   FIR_IIRFilter::eval(complex<double>   coeffs[], int np, complex<double>   z)
    ...
    complex<double>  FIR_IIRFilter::evaluate(complex<double>  topco[],complex<double>  botco[], int np, complex<double>  z)
    ...
    void FIR_IIRFilter::choosepole(complex<double>  z)
      { if (z.real() < 0.0)
    ...
    void  FIR_IIRFilter::compute_s() /* compute S-plane poles for prototype LP filter */
      {
        polemask=1;
        numpoles =0;
        if (opts & opt_be)
    ...
    void FIR_IIRFilter::compute_z() /* given S-plane poles, compute Z-plane poles */
      { int i;
        for (i=0; i < numpoles; i++)
    ...
    void FIR_IIRFilter::multin(complex<double>  w, complex<double>  coeffs[])
    ...
    void FIR_IIRFilter::expand(complex<double>  pz[],complex<double>   coeffs[])
    ...
    void FIR_IIRFilter::expandpoly() /* given Z-plane poles & zeros, compute top & bot polynomials in Z, and then recurrence relation */
    ...
    void FIR_IIRFilter::printrecurrence() /* given (real) Z-plane poles & zeros, compute & print recurrence relation */
      { int i;
        printf("Recurrence relation:\n");
        printf("y[n] = ");
        for (i=0; i < numpoles+1; i++)
